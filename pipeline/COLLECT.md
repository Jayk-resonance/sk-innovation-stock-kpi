...
대시보드 가격 기준은 **KRX 정규장 종가**다. NXT 종가나 통합시장 현재가를
사용하지 않는다.
...
- 당일 일봉은 장 마감 뒤에도 시간외 체결에 따라 임시값이 바뀔 수 있다.
  **다음 거래일 개장 전 재조회한 확정 일봉**만 원본으로 채택한다.
- `koreaStock-stock_get_quote(market_div_code="J")` 로 KRX 종가를 별도 확인한다.
  `NX` 또는 통합시장 코드는 검증이나 저장에 사용하지 않는다.
